...
Pipeline.prototype.urlToFile = function(url) {
  var file = url
    .replace("." + this.config.urlExtension, "." + this.config.fileExtension)
    .replace(this.config.urlPrefix, this.config.root)
...
Pipeline.prototype.addFiles = function(file) {
   ...
   var stats = fs.statSync(file)
    , that = this
    , files
  if(stats.isFile() && path.extname(file) === "." + this.config.fileExtension) {
